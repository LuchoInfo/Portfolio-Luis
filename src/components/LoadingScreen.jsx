import { useEffect, useState } from "react";

export const LoadingScreen = ({ onComplete }) => {
  const [text, setText] = useState("");
  const fullText = "<Hello World/>";

  useEffect(() => {
    let index = 0;
    const interval = setInterval(() => {
      setText(fullText.substring(0, index));
      index++;

      if (index > fullText.length) {
        clearInterval(interval);
        setTimeout(onComplete, 1000);
      }
    }, 100);

    return () => clearInterval(interval);
  }, [onComplete, fullText]);

  return (
    <div className="fixed inset-0 z-50 bg-black text-gray-100 flex flex-col items-center justify-center">
      {/* Contenedor principal con fuente monoespaciada */}
      <div className="mb-4 text-4xl font-bold">
        <div className="font-mono relative inline-block">
          {text}
          {/* Cursor como elemento independiente */}
          <div className="animate-blink absolute -right-3 top-0 w-[2px] h-8 bg-blue-500"></div>
        </div>
      </div>

      <div className="w-[200px] h-[2px] bg-gray-800 rounded overflow-hidden">
        <div className="w-full h-full bg-blue-500 animate-loading-bar"></div>
      </div>
    </div>
  );
};